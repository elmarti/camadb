...
const collectionCrudConformance = (
  adapterName: string,
  createContext: () => Promise<CollectionContext>,
): void => {
  describe(`${adapterName} collection CRUD conformance`, () => {
    ...
    it('enforces generated and unique identities', async () => {
      ...
      await expect(context.collection.insertOne({
        _id: 'provided',
        name: 'duplicate',
        active: false,
    ...
    it('supports count, update, delete, and upsert result contracts', async () => {
      ...
      await expect(context.collection.updateMany(
        { active: true },
        { $set: { active: false } },
      )).resolves.toMatchObject({ matchedCount: 2, modifiedCount: 2 });
      ...
      await expect(context.collection.upsert(
        { name: 'created' },
...
collectionCrudConformance('in-memory', async () => {
  ...
  return { collection, async cleanup() { await collection.destroy(); } };
});
