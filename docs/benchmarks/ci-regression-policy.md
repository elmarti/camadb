...
Because the gate evaluates many results on shared hosted hardware, an isolated statistically separated result can still occur by chance. CI therefore repeats the complete baseline/candidate pair whenever an initial comparison finds a regression. The pull request is rejected only when the independent confirmation run reproduces the same workload and result regression. A different second-run outlier is reported as unconfirmed rather than misrepresented as confirmation of the first. Confirmation uses fresh output directories, preserves both runs and the initial regression identities in the uploaded artifact, and never averages a noisy result into acceptance. Passing the retry is evidence that the first result was not reproducible, not evidence that the candidate was faster.
